  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("message", (message) => {
      ...
      if (data.nickname && !Games[Games.length - 1].clients.has(ws)) {
        ...
        const existingNicknames = Array.from(
          Games[Games.length - 1].clients.values()
        );
        if (existingNicknames.includes(data.nickname)) {
          let i = 1;
          while (
            Array.from(Games[Games.length - 1].clients.values()).includes(
              data.nickname + i
            )
          ) {
            i++;
        ...
        // const currentGame = Games[data.message.gameId];
      ...
      if (currentGame.clients.has(ws) && data.message) {
        ...
        if (data.message.messageType === "move") {
          ...
          const playerPosition = currentGame.players.get(ws); // get player's position
          ...
          if (isMoveValid(newPosition, currentGame)) {
            currentGame.players.set(ws, newPosition); // update position on server
            broadcast = {
              messageType: "move",
              id: userId,
    ...
    ws.on("close", () => {
      ...
      if (Games[ws.gameId].clients.size === 0) {
        Games.splice(ws.gameId, 1);
      }
    });
